...
class DoctorBloc extends Bloc<DoctorEvent, DoctorState> {
  ...
  DoctorBloc(this.api) : super(DoctorInitial()) {
    ...
    on<LoadDoctorsFromLocal>(_onLoadDoctorsFromLocal);
  }
  ...
  Future<void> _onLoadDoctors(
    ...
    try {
      ...
      await DatabaseHelper().clearAllDoctors();       

      // Store doctors in local database
      for (var doctor in doctors) {
        await DatabaseHelper().insertDoctor(doctor);
        print('Inserted doctor: ${doctor.name}');
      }
       
      await DatabaseHelper().printAllDoctors();
  ...
  Future<void> _onLoadDoctorsFromLocal(
      LoadDoctorsFromLocal event, Emitter<DoctorState> emit) async {
    emit(DoctorLoading());

    try {
      List<GetDoctorModel> doctors = await DatabaseHelper().getDoctors();

      print('4444444444444444444444');
      print('Loaded doctors: $doctors');

      if (doctors.isEmpty) {
        print('No doctors found in local database');
        add(LoadDoctors());
      } else {
        print('Loaded ${doctors.length} doctors from local database');
        emit(DoctorLoaded(doctors: doctors));
      }
    } catch (e) {
      print('Error fetching doctors: $e');
      emit(DoctorError(message: 'Unexpected error occurred: ${e.toString()}'));
    }
  }
